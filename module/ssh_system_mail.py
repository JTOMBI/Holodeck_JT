...
ADMIN_EMAIL = "tombijeremy@gmail.com"
...
servers = [
    {"hostname": "192.168.75.129", "username": "jtombi", "password": "salakae"},
    {"hostname": "192.168.75.132", "username": "jtombi", "password": "salakae"},
    {"hostname": "192.168.75.133", "username": "jtombi", "password": "salakae"}
]
...
try:
    ...
    # Création de la table si elle n'existe pas, avec colonne last_alert
    cursor.execute("""
...
# Fonction pour vérifier si un email a déjà été envoyé dans l'heure
def can_send_email(last_alert_time):
    if last_alert_time is None:
        return True   
    else:
        print("Non envoie de mail car 1h n'est pas passer")
    time_since_last_alert = datetime.now() - last_alert_time
    return time_since_last_alert > timedelta(hours=1)
...
def insert_data(server, cpu_usage, ram_usage, disk_usage):
    if cpu_usage is not None and ram_usage is not None and disk_usage is not None:
        ...
        # Vérification du dernier envoi d'alerte
        cursor.execute("SELECT last_alert FROM sys_res WHERE server=%s ORDER BY timestamp DESC LIMIT 1", (server["hostname"],))
        result = cursor.fetchone()
        last_alert_time = result[0] if result else None
        ...
        if (cpu_usage > CPU_THRESHOLD or ram_usage > RAM_THRESHOLD or disk_usage > DISK_THRESHOLD) and can_send_email(last_alert_time):
            send_alert_email(server, cpu_usage, ram_usage, disk_usage)
            # Mettre à jour la dernière alerte dans la base de données
            cursor.execute("UPDATE sys_res SET last_alert=%s WHERE server=%s", (timestamp, server["hostname"]))
            db.commit()
